                ...
                <div key={a.id} className="p-4 space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-white text-sm truncate">{a.titre}</p>
                      <p className="text-xs text-[#00bcd4] mt-0.5">{formatPrice(a.prix)}</p>
                    </div>
                    <span className={`flex-shrink-0 px-2 py-0.5 rounded-full text-xs font-medium ${STATUT_COLORS[a.statut] ?? 'bg-gray-100 text-gray-600'}`}>
                      {a.statut}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <Link
                      href={a.statut === 'brouillon' ? `/dashboard/agent/apercu/${a.id}` : a.statut === 'archivé' ? `/publier?edit=${a.id}` : `/bien/${a.id}`}
                      className="flex-1 text-center py-2.5 bg-[#0f172a] border border-[#334155] text-slate-300 text-xs font-semibold rounded-lg min-h-[44px] flex items-center justify-center"
                    >
                      {a.statut === 'brouillon' ? 'Aperçu' : a.statut === 'archivé' ? 'Modifier' : 'Voir'}
                    </Link>
                    <Link
                      href={`/publier?edit=${a.id}`}
                      className="flex-1 text-center py-2.5 bg-[#00bcd4]/10 border border-[#00bcd4]/30 text-[#00bcd4] text-xs font-semibold rounded-lg min-h-[44px] flex items-center justify-center"
                    >
                      Modifier
                    </Link>
                    {a.statut === 'brouillon' && (
                      <button
                        ...
                        className="flex-1 py-2.5 bg-green-600 text-white text-xs font-bold rounded-lg min-h-[44px]"
                      >
                        Publier
                      </button>
                    )}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[#0f172a]">
                    ...
                    <th className="text-left px-3 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Ville</th>
                    <th className="text-left px-3 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide">Vues</th>
                    <th className="text-left px-3 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Statut</th>
                ...
                <tbody className="divide-y divide-[#334155]">
                  {displayedAnnonces.map(a => (
                    <tr key={a.id} className="hover:bg-[#0f172a] transition-colors">
                      ...
                      <td className="px-3 py-3.5 text-gray-400">{a.ville}</td>
