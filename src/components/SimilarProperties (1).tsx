...
export default async function SimilarProperties({ currentId, typeBien, ville, transaction, agentId }: Props) {
  ...
  // ── Critère 3 : même ville ──
  if (ville) {
    const { data: byVille } = await supabase
      .from('biens')
      .select(SELECT_FIELDS)
      .neq('id', currentId)
      .eq('statut', 'publié')
      .eq('localites.ville', ville)
      .limit(12)
    // Le filtre sur une relation imbriquée sans !inner peut renvoyer localites=null
    // pour les biens ne correspondant pas — on filtre donc manuellement en plus.
    addResults((byVille ?? []).filter((b: any) => b.localites?.ville === ville))
  }

  // Score de pertinence : nombre de critères correspondants
  for (const b of pool) {
    let score = 0
