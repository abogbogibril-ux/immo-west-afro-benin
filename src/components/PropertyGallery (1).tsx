...
export default function PropertyGallery({ images, titre, bienId, userId, isFavorited = false }: Props) {
  ...
  // ── Authentification détectée côté client (le serveur ne peut pas voir la session) ──
  const [actualUserId, setActualUserId] = useState<string | undefined>(userId)
  ...
  const toggleFavori = async (e: React.MouseEvent) => {
    e.stopPropagation()
    if (!actualUserId) { router.push(`/connexion?redirect=/bien/${bienId}`); return }
    ...
    try {
      if (nextState) {
        const { error } = await supabase.from('favoris').insert({ user_id: actualUserId, bien_id: bienId })
        if (error) throw error
      } else {
        const { error } = await supabase.from('favoris').delete().eq('user_id', actualUserId).eq('bien_id', bienId)
        if (error) throw error
      }
    } catch {
      setFavorited(!nextState)
