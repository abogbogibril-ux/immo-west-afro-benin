...
export default function ClientMessagesPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: envoyes } = await supabase
        .from('messages')
        .select(`*, profiles!messages_destinataire_id_fkey (nom, prenom, telephone, avatar_url), biens (titre)`)
        .eq('expediteur_id', user.id)
        .order('created_at', { ascending: false })

      const all = [
        ...(recus ?? []).map((m: any) => ({ ...m, _type: 'recu' })),
        ...(envoyes ?? []).map((m: any) => ({ ...m, _type: 'envoye' })),
  ...
  const sendReply = async () => {
    ...
    setSending(true)
    const destinataire = (selected as any)._type === 'recu'
  ...
  const unreadCount = messages.filter((m: any) => !m.lu && m._type === 'recu').length

  const getContact = (msg: any) => msg.profiles
            ...
            <div className="bg-white border-t border-gray-100 p-4">
              <div className="flex gap-3 items-end max-w-2xl">
                ...
                <button onClick={sendReply} disabled={sending || !reply.trim()}
                  className="flex items-center gap-2 px-4 py-2.5 bg-green-600 text-white text-sm font-semibold rounded-xl hover:bg-green-700 transition-colors disabled:opacity-50 flex-shrink-0">
                  {sending ? (
