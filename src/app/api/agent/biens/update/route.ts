...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const { bienId, updates } = await req.json()
    if (!bienId || !updates) return NextResponse.json({ error: "Parametres manquants" }, { status: 400 })

    // Verifier que le bien appartient a cet agent (sauf admin)
    ...
    const { error } = await supabaseAdmin
      .from("biens").update(updates).eq("id", bienId)
