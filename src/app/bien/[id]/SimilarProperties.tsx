...
export default async function SimilarProperties({ currentId, typeBien, ville, transaction, agentId }: Props) {
  ...
  // Requêtes indépendantes lancées EN PARALLÈLE — type, agent, ville, transaction
  const [byTypeRes, byAgentRes, byVilleRes, byTxRes] = await Promise.all([
    supabase.from('biens').select(SELECT_FIELDS)
      .neq('id', currentId).eq('statut', 'publié').eq('type_bien', typeBien).limit(12),
    agentId
      ? supabase.from('biens').select(SELECT_FIELDS)
          .neq('id', currentId).eq('statut', 'publié').eq('agent_id', agentId).limit(12)
      : Promise.resolve({ data: null }),
    ville
      ? supabase.from('biens').select(SELECT_FIELDS)
          .neq('id', currentId).eq('statut', 'publié').eq('ville', ville).limit(12)
      : Promise.resolve({ data: null }),
    supabase.from('biens').select(SELECT_FIELDS)
      .neq('id', currentId).eq('statut', 'publié').eq('transaction', transaction).limit(12),
  ])
  ...
  addResults(byTypeRes.data)
  addResults(byAgentRes.data)
  addResults(byVilleRes.data)
  if (pool.length < 3) addResults(byTxRes.data)
  ...
  for (const b of pool) {
    ...
    if (b.transaction === transaction) score++
