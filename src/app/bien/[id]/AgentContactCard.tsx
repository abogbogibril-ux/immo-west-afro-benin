...
export default function AgentContactCard({
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
      ...
      {/* Prix */}
      <div className="bg-gradient-to-br from-green-600 to-emerald-700 px-5 py-5">
        ...
        <div className="text-white">
          <span className="text-2xl md:text-3xl font-bold">{formattedPrice}</span>
          <span className="text-green-200 text-sm ml-1">{prixSuffix}</span>
        </div>
        {surface && (
      ...
      <div className="px-5 py-4 flex flex-col gap-4 flex-1">
        ...
        <div className="flex items-center gap-2">
          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${statConfig.color}`}>
        ...
        {agent && (
          <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
            {agent.avatar_url ? (
              <img src={agent.avatar_url} alt="" className="w-11 h-11 rounded-full object-cover ring-2 ring-green-100" />
            ) : (
              <div className="w-11 h-11 rounded-full bg-gradient-to-br from-green-400 to-emerald-600 flex items-center justify-center text-white font-bold text-base shadow-sm flex-shrink-0">
                {(agent.prenom?.[0] ?? '').toUpperCase()}{(agent.nom?.[0] ?? '').toUpperCase()}
              </div>
            )}
            <div className="min-w-0">
              <p className="font-semibold text-gray-900 text-sm truncate">{agent.prenom} {agent.nom}</p>
        ...
        <div className="flex flex-col gap-2.5">
          {whatsappNum && (
            <a href={`https://wa.me/${whatsappNum.replace(/[\s+\-()]/g, '')}?text=${waText}`}
              ...
              className="flex items-center justify-center gap-2.5 w-full py-3 bg-[#25D366] text-white rounded-xl font-semibold text-sm hover:bg-[#1fba59] transition-colors shadow-sm">
          ...
          {agent?.telephone && (
            <a href={`tel:${agent.telephone}`}
              className="flex items-center justify-center gap-2.5 w-full py-3 bg-green-600 text-white rounded-xl font-semibold text-sm hover:bg-green-700 transition-colors shadow-sm">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 7V5z" />
              </svg>
              Appeler : {agent.telephone}
            </a>
          )}
          ...
          {!visitSent ? (
            !showVisit ? (
              <button onClick={() => setShowVisit(true)}
                className="flex items-center justify-center gap-2.5 w-full py-3 bg-white border-2 border-gray-200 text-gray-700 rounded-xl font-semibold text-sm hover:border-green-400 hover:text-green-700 transition-all">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setShowVisit(false)}
                    className="flex-1 py-2.5 border border-gray-200 text-gray-500 rounded-xl text-sm hover:bg-gray-50 font-medium">
                    Annuler
                  </button>
                  <button onClick={sendVisitRequest} disabled={loading}
                    className="flex-1 py-2.5 bg-green-600 text-white rounded-xl text-sm font-semibold hover:bg-green-700 disabled:opacity-60 flex items-center justify-center gap-1.5">
                    {loading ? (
            ...
            <div className="flex items-center gap-2 p-3 bg-green-50 rounded-xl border border-green-100">
              ...
              <p className="text-sm text-green-700 font-medium">Demande de visite envoyée !</p>
            </div>
          )}
        </div>

        <BienDisclaimer annonceurNom={agent ? agent.prenom + ' ' + agent.nom : undefined} />
