...
export default function PropertyGallery({ images, titre, bienId, userId, isFavorited = false }: Props) {
  ...
  const shareOptions = [
    ...
    },
    {
      label: 'Email',
      icon: 'email',
      action: () => { window.location.href = `mailto:?subject=${shareText}&body=${encodeURIComponent(shareUrl)}` },
    },
    {
      label: 'Facebook',
      icon: 'facebook',
                ...
                <div className="absolute bottom-full mb-2 right-0 bg-white dark:bg-gray-900 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 py-2 min-w-[180px] z-30">
                  {shareOptions.map(opt => (
                    <button key={opt.label}
                      ...
                      {opt.icon === 'facebook' && (
                        <svg className="w-4 h-4 text-[#1877F2] flex-shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M9.101 23.691v-7.98H6.627v-3.667h2.474v-1.58c0-4.085 1.848-5.978 5.858-5.978.759 0 2.068.15 2.605.298v3.325c-.283-.03-.774-.045-1.386-.045-1.967 0-2.727.745-2.727 2.686v1.294h3.925l-.674 3.667h-3.25v8.245C19.396 23.238 24 18.179 24 12.044c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.628 3.874 10.35 9.101 11.647z"/></svg>
                      )}
                      {opt.icon === 'copy' && (
